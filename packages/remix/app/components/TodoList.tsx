...
const TodoList = ({ todos }: { todos: Awaited<ReturnType<typeof getAllTodos>> }) => {
    ...
    <div className="overflow-hidden bg-white dark:bg-gray-800 shadow sm:rounded-md">
      <ul role="list" className="divide-y divide-gray-200 dark:divide-gray-700">
        {todos.map((todo) => (
          <li
            key={todo.id}
            onClick={() => deleteTodo.submit({ todoId: todo.id.toString(), intent: "delete" }, { method: "post" })}
          >
              ...
              <div className="px-4 py-4 sm:px-6">
                ...
                <div className="mt-2 sm:flex sm:justify-end">
                  <div className="mt-2 flex items-center text-sm text-gray-500 sm:mt-0 dark:text-gray-300">
                    ...
                    />
                    <p>
                      Created at <time dateTime={todo.createdAt}>{new Date(todo.createdAt).toLocaleTimeString()}</time>
                    </p>
                  </div>
                </div>
              </div>
